...
def render_eda(df, feature_names, target_name, target_labels, dataset_info):
    ...
    col3.metric("🏷️ Jumlah Kelas", dataset_info['n_classes'])
...
def _render_data_preview(df, feature_names, dataset_info):
    ...
    if missing.sum() > 0:
        st.warning(f"⚠️ Terdapat {missing.sum()} missing values!")
...
def _render_distributions(df, feature_names, target_name, target_labels):
    ...
    # Create label column for coloring
    df_plot = df.copy()
...
def _render_target_distribution(df, target_name, target_labels):
    ...
    st.markdown("#### 🏷️ Distribusi Target/Label")
    ...
    if ratio < 0.5:
        st.warning(
            f"⚠️ Dataset **tidak seimbang** (rasio = {ratio:.2f}). "
